...
tests for `print_line_padding_right_just` in `kamilog.py`
"""

import io
from kamilog import print_line_padding_right_just
...
class TestLinePaddingRightJust:
    ...
    def test_content_fills_exactly(_):
        out = io.StringIO()
        print_line_padding_right_just("hi", "=", line_width=2, file=out)
        assert out.getvalue() == "hi\n"
    ...
    def test_custom_end(_):
        out = io.StringIO()
        print_line_padding_right_just("hi", "=", line_width=10, end="", file=out)
        assert out.getvalue() == "========hi"

    def test_custom_file(_):
        out = io.StringIO()
        print_line_padding_right_just("test", "#", line_width=12, file=out)
